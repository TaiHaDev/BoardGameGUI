...
// author Phuoc
public class AIViewer extends Application implements Initializable {

    private final FXMLLoader fxmlLoader = new FXMLLoader(Game.class.getResource("fxml/AI-viewer.fxml"));
    ...
    Player player1 = new Player("W", Color.RED);
    Player player2 = new Player("X", Color.BLUE);
    Player player3 = new Player("Y", Color.GREEN);
    GameInstance game = new GameInstance(new Player[] {player1, player2, player3});
    AIPlayer ai1 = new SmartNoExpAI(game.getPlayers().get(0));
    AIPlayer ai2 = new SmartAI(game.getPlayers().get(1));
    AIPlayer ai3 = new GreedyAI(game.getPlayers().get(2));
    ...
    public void AIFight(GameInstance game) {
        try {
            AIFighterUtil.AIFight(game, ai1, ai2, ai3);
...
//        clearBoard(gameInstance);
        ...
        gameInstance.getPlayers()
                .forEach(player -> (player.getUniqueId().equals("X") ? playerXColor :
                        (player.getUniqueId().equals("Y") ? playerYColor : playerWColor))
